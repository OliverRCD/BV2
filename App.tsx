...
import { validateApiKey, setRuntimeApiKey } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleVerifyKey = async () => {
      ...
      if (isValid) {
          setKeyStatus('valid');
          // FIX: Use the service method to set the key, do not rely on process.env in production
          setRuntimeApiKey(keyInput);
      } else {
          setKeyStatus('invalid');
          setRuntimeApiKey(''); // Clear potentially invalid key
      }
  };
